...
extern int tinyobj_parse_mtl_file(tinyobj_material_t **materials_out, size_t *num_materials_out, const char *filename);
...
int tinyobj_parse_mtl_file(tinyobj_material_t **materials_out, size_t *num_materials_out, const char *filename) {
...
static int parseLine(Command *command, const char *p, size_t p_len, int triangulate) {
  ...
  if ((0 == strncmp(token, "usemtl", 6)) && IS_SPACE((token[6]))) {
    ...
    command->material_name_len = (unsigned int)length_until_newline(command->material_name, (p_len - (size_t)(token - linebuf)) + 1);
  ...
  if ((0 == strncmp(token, "mtllib", 6)) && IS_SPACE((token[6]))) {
    ...
    command->mtllib_name_len = (unsigned int)length_until_newline(command->mtllib_name, p_len - (size_t)(token - linebuf)) + 1;
  ...
  if (token[0] == 'g' && IS_SPACE((token[1]))) {
    ...
    command->group_name_len = (unsigned int)length_until_newline( command->group_name, p_len - (size_t)(token - linebuf)) + 1;
  ...
  if (token[0] == 'o' && IS_SPACE((token[1]))) {
    ...
    command->object_name_len = (unsigned int)length_until_newline(command->object_name, p_len - (size_t)(token - linebuf)) + 1;
