...
int make_mesh_and_material_by_obj(vertex_t **mesh, ulong *mesh_num, int **material_ids, ulong *material_ids_num, const char *name) 
{
	...
	int ret = tinyobj_parse_obj(&attrib, &shapes, &num_shapes, &tmaterials, &num_materials, buffer, data_len, flags, prefix_path);
    ...
    for (i = 0; i < attrib.num_face_num_verts; i++) 
	{
        ...
        for (f = 0; f < (size_t)attrib.face_num_verts[i] / 3; f++) 
		{
            ...
            if (attrib.num_normals > 0) 
			{
				...
				else  /* normal index is not defined for this face, compute geometric normal */
				{ 
